...
const hasUploadReferenceStatement = db.prepare(`
    ...
    LIMIT 1
  `);
export function parseUploadUrl(url) {
    ...
    if (!userId || !filename || filename.includes('\0'))
        return null;
    if (filename !== path.posix.basename(filename))
        return null;
...
export function isUploadUrlReferenced(url) {
    return Boolean(hasUploadReferenceStatement.get({ url }));
}
export function collectUploadUrlsFromSpace(space) {
    if (!space)
        return [];
    const urls = new Set();
    const addUrl = (value) => {
...
function removeFile(filePath) {
    ...
    if (path.resolve(parentDir) === path.resolve(UPLOADS_DIR))
        return;
    try {
...
export function pruneStaleOrphanedUploads() {
    ...
    while (stack.length > 0) {
        ...
        for (const entry of entries) {
            ...
            if (isUploadUrlReferenced(encodedUrl))
                continue;
            removeFile(fullPath);
            deletedCount += 1;
        }
